...
class SceneUnderstanding:
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            logger.info("Scene classification model initialized successfully")
            
        except Exception as e:
            logger.error(f"Error initializing scene classification model: {str(e)}")
            raise

    def _load_categories(self):
        """Load Places365 categories."""
        try:
            categories_file = Path(__file__).parent / 'models' / 'categories_places365.txt'
            categories_file.parent.mkdir(parents=True, exist_ok=True)
            
            if not categories_file.exists():
                try:
                    # Download categories file if it doesn't exist
                    ...
                    logger.warning(f"Could not download categories file: {str(e)}")
                    # Create a basic set of categories if download fails
                    self._create_basic_categories(categories_file)
            ...
            logger.info(f"Loaded {len(self.categories)} scene categories")
            
        except Exception as e:
            logger.error(f"Error loading scene categories: {str(e)}")
            # Create basic categories as fallback
            self._create_basic_categories(categories_file)
            logger.info("Using basic scene categories as fallback")

    def _create_basic_categories(self, categories_file):
        """Create a basic set of scene categories as fallback."""
        basic_categories = [
            "indoor", "outdoor", "building", "street", "nature",
            "room", "kitchen", "bathroom", "bedroom", "living_room",
            "office", "classroom", "restaurant", "store", "gym",
            "park", "beach", "mountain", "forest", "garden",
            "city", "suburb", "rural", "industrial", "commercial"
        ]
        
        with open(categories_file, 'w') as f:
            for i, category in enumerate(basic_categories):
                f.write(f"/a/{category}\n")
        
        self.categories = {i: category for i, category in enumerate(basic_categories)}
        logger.info("Created basic scene categories as fallback")
    ...
    def process_scenes(self, video_path):
        ...
        try:
            ...
            while cap.isOpened():
                ...
                scene_features.append(frame_results)
